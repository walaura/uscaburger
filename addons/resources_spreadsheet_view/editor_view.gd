...
func save_data():
	if node_recent_paths.recent_paths.is_empty() and node_columns.column_properties.is_empty() and table_functions_dict.is_empty():
		return
...
func can_display_property(property_info: Dictionary):
	...
	return prop_type != TYPE_CALLABLE and prop_type != TYPE_SIGNAL and property_info[&"usage"] & PROPERTY_USAGE_EDITOR != 0 and property_info[&"name"] != "script"
...
func column_can_solo_open(column_index: int) -> bool:
	return column_types[column_index] == TYPE_OBJECT or (column_types[column_index] == TYPE_ARRAY and column_hint_strings[column_index][0].begins_with("24"))
...
func _update_row(row_index: int, color_rows: bool = true):
	...
	for i in columns.size():
		if node_table_root.get_child_count() <= (row_index - first_row) * columns.size() + i:
			...
			current_node.tooltip_text = (columns[i].capitalize() + "\n---\n" + "Of " + shortened_path)
...
func set_edited_cells_values(new_cell_values: Array):
	...
	editor_plugin.undo_redo.add_undo_method(self, &"_update_resources", edited_cells_resources.duplicate(), edited_rows.duplicate(), column, get_edited_cells_values())
	editor_plugin.undo_redo.add_undo_method(_selection, &"_update_selected_cells_text")
	editor_plugin.undo_redo.add_do_method(self, &"_update_resources", edited_cells_resources.duplicate(), edited_rows.duplicate(), column, new_cell_values.duplicate())
	...
	for i in update_rows.size():
		...
		if column_types[update_column] == TYPE_COLOR:
			for j in columns.size() - update_column:
				...
				_selection.column_editors[j + update_column].set_color(_selection.get_cell_node_from_position(Vector2i(update_column + j, row)), values[i])
