...
func _play_intro() -> void:
	...
	for line in CurrentRunState.score_handler.last_settled_score:
		...
		tween.tween_property(tkt_line, "scale", Vector2.ONE, .15 * speed_mult).from(Vector2.ONE * 1.1)
...
func _on_next_round_pressed() -> void:
	...
	tween.parallel().tween_property((%UIContainer as Control), "offset_transform_position:y", -300., .5)
	tween.finished.connect(
		func() -> void:
			get_tree().paused = false
			on_next_round.emit()
